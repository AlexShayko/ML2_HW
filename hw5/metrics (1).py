...
def compute_precision(reference: List[LabeledAlignment], predicted: List[List[Tuple[int, int]]]) -> Tuple[int, int]:
    ...
    for i in range(len(reference)):
        possible = set(reference[i].possible) | set(reference[i].sure)
        pred = set(predicted[i])

        intersection += len(pred & possible)
...
def compute_aer(reference: List[LabeledAlignment], predicted: List[List[Tuple[int, int]]]) -> float:
    ...
    """
    inter_pos, total_pred = compute_precision(reference, predicted)
    inter_sure, sure_pred = compute_recall(reference, predicted)
    aer = 1 - (inter_sure + inter_pos)/(total_pred + sure_pred)
    return aer
