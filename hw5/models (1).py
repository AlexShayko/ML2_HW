...
class WordAligner(BaseAligner):
    ...
    def _e_step(self, parallel_corpus: List[TokenizedSentencePair]) -> List[np.array]:
        ...
        """
        posteriors = []
        for texts in parallel_corpus:
            source_tokens = texts.source_tokens
            target_tokens = texts.target_tokens
            submatrix = self.translation_probs[source_tokens[:, None], target_tokens]

            column_sums = submatrix.sum(axis=0)
            mask = column_sums > 0

            posterior = np.zeros_like(submatrix)
            posterior[:, mask] = submatrix[:, mask] / column_sums[mask]
    ...
    def _compute_elbo(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]) -> float:
        ...
        for i in range(len(parallel_corpus)):
            source_tokens = parallel_corpus[i].source_tokens
            target_tokens = parallel_corpus[i].target_tokens
            submatrix = self.translation_probs[source_tokens[:, None], target_tokens]
            q = posteriors[i]
            mask = q > 0
            elbo += np.sum(q[mask] * np.log(np.maximum(submatrix[mask] / q[mask], 1e-12)))
            elbo -= len(target_tokens) * np.log(len(source_tokens))
    ...
    def _m_step(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]):
        ...
        for i in range(len(parallel_corpus)):
            source_tokens = parallel_corpus[i].source_tokens
            target_tokens = parallel_corpus[i].target_tokens
            np.add.at(self.translation_probs, (source_tokens[:, None], target_tokens), posteriors[i])

        row_sums = self.translation_probs.sum(axis=1, keepdims=True)
        safe_row_sums = np.where(row_sums > 0, row_sums, 1.0)

        self.translation_probs /= safe_row_sums
    ...
    def align(self, sentences):
        ...
        for texts in sentences:
            ...
            best_preds_for_targets = np.argmax(submatrix, axis=0)

            sentence_alignment = []
            for tgt, best_src in enumerate(best_preds_for_targets):
                sentence_alignment.append((best_src + 1, tgt + 1))

            all_alignments.append(sentence_alignment)
